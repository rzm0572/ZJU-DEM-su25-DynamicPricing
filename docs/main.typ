...
我们主要对卖家的遗憾值进行了测试，考虑到运行时间，我们设置 $N=5, m=2$。

=== 非对抗情形

非对抗性情况下，采用 UCB 算法得到的理论遗憾值公式为 $tilde(cal(O)) (m sqrt(T))$，为了验证这一公式，我们在保证买家数据生成器相同的情况下，改变 $T$ 值，得到以下遗憾值曲线：

#image("assets/random_buyer_smooth.png")

#image("assets/random_buyer_adv.png")

从上两图中我们可以看出，随着 $T$ 的增加，遗憾值次线性增长，与理论公式相符。

=== 对抗情形

非对抗性情况下，采用 UCB 算法得到的理论遗憾值公式为 $tilde(cal(O)) (m sqrt(T log(|tilde(cal(P)|))))$，为了验证这一公式，我们在保证买家数据生成器相同的情况下，改变 $T$ 值，得到以下遗憾值曲线：

#image("assets/test_result.png")

从上两图中我们可以看出，随着 $T$ 的增加，遗憾值增长，但增长速度不明显。
