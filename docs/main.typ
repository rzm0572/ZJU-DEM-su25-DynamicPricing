...
以上步骤和算法 1 完全相同。然后把 $[0,N]$ 划分为均匀网络，m-step 函数的跳跃点仅取这个划分中的点，离散化间隔 $delta = floor(frac(epsilon N,m L))$表示划分的长度。
...
#proof[
    ...
    按照之前的做法，相当于 $(frac(e (N-1),m))^m dot (e ceil(2+epsilon) ceil(log_(1+epsilon)(1/epsilon)))^m $中，$(e ceil(2+epsilon) ceil(log_(1+epsilon)(1/epsilon)))^m$ 仍然被放缩为  $epsilon^(-m)$，$(frac(e (N-1),m))^m$ 放缩为 $N^m$，但是这里的 $N$ 变成了 $|N_D|=cal(O) (frac(2 J m,epsilon^2))$，放缩为 $cal(O) ((frac(J,epsilon^2))^m)$，
